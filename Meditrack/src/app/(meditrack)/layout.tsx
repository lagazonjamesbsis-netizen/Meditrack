...
const navItems = [
  { href: '/homepage', label: 'Homepage', icon: '/icon-home.png' },
  { href: '/appointmentmanagement', label: 'Appointment Management', icon: '/icon-calendar.png' },
  { href: '/events', label: 'Events', icon: '/icon-events.png' },
  { href: '/services', label: 'Services', icon: '/icon-services.png' },
  { href: '/patientrecord', label: 'Patient Record', icon: '/icon-patients.png' },
]
    ...
    <>
        ...
        <nav className="flex-1 mt-4">
          ...
          <ul className="list-none p-0 m-0 flex flex-col gap-1">
            {navItems.map((item) => {
              const isActive = pathname === item.href || (item.href !== '/homepage' && (pathname.startsWith(item.href + '/') || pathname.startsWith(item.href)))
              return (
  ...
  return (
    <>
      ...
      <div className={`absolute top-[70px] right-0 w-[260px] rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.15)] z-[200] overflow-hidden animate-[slideIn_0.2s_ease] ${darkMode ? 'bg-[#050617]' : 'bg-white'}`}>
        ...
        <div className="py-2">
          <button className={`flex items-center gap-3 w-full px-5 py-2.5 border-none bg-transparent text-sm font-semibold cursor-pointer font-poppins text-left ${darkMode ? 'text-[#F9FAFB] hover:bg-[#0f1438]' : 'text-[#2A2E43] hover:bg-gray-50'}`} onClick={() => { router.push('/profile'); onClose() }}>
          ...
          <button className={`flex items-center gap-3 w-full px-5 py-2.5 border-none bg-transparent text-sm font-semibold cursor-pointer font-poppins text-left ${darkMode ? 'text-[#F9FAFB] hover:bg-[#0f1438]' : 'text-[#2A2E43] hover:bg-gray-50'}`} onClick={() => { router.push('/accountsettings'); onClose() }}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`w-4.5 h-4.5 flex-shrink-0 ${darkMode ? 'text-[#F9FAFB]' : 'text-gray-500'}`}>
