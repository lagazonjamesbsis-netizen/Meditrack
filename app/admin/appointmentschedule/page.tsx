...
interface WalkInEntry {
  ...
  time: string
  reason: string
  accountType: 'With Account' | 'Without Account'
}
...
const initialWalkIns: WalkInEntry[] = [
  { name: 'BALTAZAR, Elena R.', ptn: 'PTN-2610250', service: 'Basic Consultation', time: '7:00am to 8:00am', reason: 'Persistent headache and dizziness', accountType: 'With Account' },
  { name: 'NAVARRO, Ramon T.', ptn: 'PTN-2610251', service: 'Vaccination', time: '9:00am to 10:00am', reason: 'Influenza vaccination for elderly', accountType: 'Without Account' },
  { name: 'AGUILAR, Liza M.', ptn: 'PTN-2610252', service: 'Check-up', time: '10:00am to 11:00am', reason: 'Annual medical check-up', accountType: 'With Account' },
  { name: 'SALAZAR, Mario D.', ptn: 'PTN-2610253', service: 'Pre-natal Care', time: '1:00pm to 2:00pm', reason: 'Prenatal check-up, 5 months pregnancy', accountType: 'Without Account' },
  { name: 'OCAMPO, Fe B.', ptn: 'PTN-2610254', service: 'Dental Care', time: '2:00pm to 3:00pm', reason: 'Tooth extraction consultation', accountType: 'With Account' },
]
...
export default function AppointmentSchedulePage() {
  ...
  const [walkInsToday, setWalkInsToday] = useState<WalkInEntry[]>(initialWalkIns)
  ...
  const handleWalkInSubmit = () => {
    ...
    setWalkInsToday(prev => [{
      name: fullName,
      ptn: `PTN-${seq}`,
      service: walkInForm.service,
      time: walkInForm.time,
      reason: walkInForm.complaint.trim() || 'Not specified',
      accountType: walkInMode === 'with-account' ? 'With Account' : 'Without Account',
    }, ...prev])
  ...
  useEffect(() => {
    document.body.style.overflow = viewingPatient !== null || showWalkIn ? 'hidden' : ''
    return () => { document.body.style.overflow = '' }
  }, [viewingPatient, showWalkIn])

  const notifyPatient = (ptn: string) => {
    setNotified(prev => new Set(prev).add(ptn))
    toast.success('Patient notified successfully')
    setTimeout(() => setNotified(prev => { const next = new Set(prev); next.delete(ptn); return next }), 3000)
  }

  const openRecord = (ptn: string) => {
    const record = initialRecords.find(r => r.id === ptn)
    if (record) {
  ...
  return (
    <div>
      <h1 className={`text-[30px] sm:text-[38px] lg:text-[45px] ${darkMode ? 'text-[#F9FAFB]' : 'text-[#1d4662]'} my-0 mb-[14px] text-left`}>Appointment Schedule</h1>

      <div className={`${darkMode ? 'bg-[#2d1b4e]' : 'bg-white'} border ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-[rgba(15,60,95,0.08)]'} p-4 sm:p-6 rounded-[24px] mb-7 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.06)]`}>
        <h2 className={`text-[26px] sm:text-[32px] lg:text-[40px] ${darkMode ? 'text-[#F9FAFB]' : 'text-[#1d4662]'} m-0 text-center`}>Today's Schedule</h2>
        <p className={`text-center text-[15px] m-0 mt-1 mb-[18px] ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{fmtLong(now)}</p>
        <div className={`overflow-hidden rounded-2xl border ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-gray-200'}`}>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-[16px] min-w-[760px]" style={{ tableLayout: 'fixed' }}>
              <thead>
                <tr className="bg-[#4E69D3]">
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[30%]">Patient Name</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[14%]">ID</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[16%]">Service</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[16%]">Time</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[24%]">Actions</th>
                </tr>
              </thead>
              <tbody>
                {schedule.slice((todayPage - 1) * 10, todayPage * 10).map(p => (
                  <tr key={p.ptn} className={`border-b last:border-0 transition-colors ${darkMode ? 'odd:bg-[#0f1438] even:bg-white/[0.04] border-white/[0.06] hover:bg-[#3d2768]' : 'odd:bg-white even:bg-[#F6F8FF] border-[#EEF2F7] hover:bg-[#EEF0FB]'}`}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3 overflow-hidden">
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 ${p.relativeOf ? 'bg-[#E8EAF6] text-[#4E69D3]' : darkMode ? 'bg-[#4E69D3]/25 text-blue-200' : 'bg-[#E8EAF6] text-[#4E69D3]'}`}>{p.name.charAt(0)}</div>
                        <div className="min-w-0">
                          <div className={`text-[16px] font-poppins font-semibold truncate ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`} title={p.name}>{p.name}</div>
                          {p.relativeOf && <div className={`text-[12px] font-semibold italic truncate ${darkMode ? 'text-[#C4B5FD]' : 'text-[#7C3AED]'}`}>Relative of {p.relativeOf}</div>}
                        </div>
                      </div>
                    </td>
                    <td className={`px-6 py-4 text-[16px] font-semibold font-mono ${darkMode ? 'text-[#C4B5FD]' : 'text-[#4E69D3]'}`}>{p.ptn}</td>
                    <td className="px-6 py-4">
                      <span style={{ background: sc(p.service).bg, color: sc(p.service).color }} className="inline-block px-3 py-1 rounded-full text-[12px] font-bold whitespace-nowrap">{sc(p.service).label}</span>
                    </td>
                    <td className={`px-6 py-4 text-[16px] font-medium whitespace-nowrap ${darkMode ? 'text-[#F9FAFB]' : 'text-gray-700'}`}>{p.time}</td>
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        <button onClick={() => openRecord(p.ptn)} className={`bg-transparent border rounded-lg px-3 py-2 text-xs font-semibold cursor-pointer transition-colors ${darkMode ? 'text-[#F9FAFB] border-white/30 hover:bg-white/10' : 'text-[#4E69D3] border-[#4E69D3] hover:bg-[#EEF0FB]'}`}>View Record</button>
                        <button onClick={() => notifyPatient(p.ptn)} className={`border-none rounded-lg px-3 py-2 text-xs font-semibold cursor-pointer transition-colors ${notified.has(p.ptn) ? 'bg-green-600 text-white' : 'bg-[#4E69D3] text-white hover:bg-[#3D56B8]'}`}>{notified.has(p.ptn) ? 'Sent!' : 'Notify'}</button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        {Math.ceil(schedule.length / 10) > 1 && (
          <div className="flex justify-end mt-5">
            <Pagination darkMode={darkMode} page={todayPage} totalPages={Math.ceil(schedule.length / 10)} onPage={setTodayPage} />
          </div>
        )}
      </div>

      <div className={`${darkMode ? 'bg-[#2d1b4e]' : 'bg-white'} border ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-[rgba(15,60,95,0.08)]'} p-4 sm:p-6 rounded-[24px] mb-7 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.06)]`}>
        <h2 className={`text-[26px] sm:text-[32px] lg:text-[40px] ${darkMode ? 'text-[#F9FAFB]' : 'text-[#1d4662]'} m-0 text-center`}>Today's Walk-ins</h2>
        <p className={`text-center text-[15px] m-0 mt-1 mb-[18px] ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{fmtLong(now)}</p>
        <div className={`overflow-hidden rounded-2xl border ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-gray-200'}`}>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-[16px] min-w-[820px]" style={{ tableLayout: 'fixed' }}>
              <thead>
                <tr className="bg-green-600">
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[24%]">Patient Name</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[13%]">ID</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[14%]">Service</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[12%]">Time</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[22%]">Reason for Visit</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[15%]">Account Type</th>
                </tr>
              </thead>
              <tbody>
                {walkInsToday.length === 0 ? (
                  <tr>
                    <td colSpan={6} className={`px-6 py-10 text-center text-[16px] ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>No walk-ins registered yet today.</td>
                  </tr>
                ) : walkInsToday.map(w => (
                  <tr key={w.ptn} className={`border-b last:border-0 transition-colors ${darkMode ? 'odd:bg-[#0f1438] even:bg-white/[0.04] border-white/[0.06] hover:bg-[#3d2768]' : 'odd:bg-white even:bg-[#F6F8FF] border-[#EEF2F7] hover:bg-[#EEF0FB]'}`}>
                    <td className={`px-6 py-4 text-[16px] font-poppins font-semibold truncate ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}>{w.name}</td>
                    <td className={`px-6 py-4 text-[16px] font-semibold font-mono ${darkMode ? 'text-[#C4B5FD]' : 'text-[#4E69D3]'}`}>{w.ptn}</td>
                    <td className="px-6 py-4">
                      <span style={{ background: sc(w.service).bg, color: sc(w.service).color }} className="inline-block px-3 py-1 rounded-full text-[12px] font-bold whitespace-nowrap">{sc(w.service).label}</span>
                    </td>
                    <td className={`px-6 py-4 text-[16px] font-medium whitespace-nowrap ${darkMode ? 'text-[#F9FAFB]' : 'text-gray-700'}`}>{w.time}</td>
                    <td className={`px-6 py-4 text-[16px] font-medium leading-relaxed ${darkMode ? 'text-[#F9FAFB]' : 'text-gray-700'}`}>{w.reason}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-block px-3 py-1 rounded-full text-[12px] font-bold whitespace-nowrap ${w.accountType === 'With Account' ? 'bg-[#DCFCE7] text-[#16A34A]' : 'bg-[#FEF3C7] text-[#975A16]'}`}>{w.accountType}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className={`flex items-center gap-2 mt-4 px-5 py-3 rounded-xl text-[13px] font-semibold ${darkMode ? 'bg-[#0f1438] text-[#C4B5FD] border border-[rgba(255,255,255,0.10)]' : 'bg-[#f8fbff] text-[#4E69D3] border border-[rgba(15,60,95,0.10)]'}`}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4 flex-shrink-0"><circle cx="12" cy="12" r="10" /><line x1="12" y1="16" x2="12" y2="12" /><line x1="12" y1="8" x2="12.01" y2="8" /></svg>
          Walk-ins registered <b>Without Account</b> create new user accounts and will appear in <b>User Management</b> for approval.
        </div>
      </div>
      ...
      <div className={`${darkMode ? 'bg-[#2d1b4e]' : 'bg-white'} border ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-[rgba(15,60,95,0.08)]'} p-4 sm:p-6 rounded-[24px] mb-7 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.06)]`}>
        ...
        <div className={`overflow-hidden rounded-2xl border ${darkMode ? 'border-[rgba(255,255,255,0.10)]' : 'border-gray-200'}`}>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-[16px] min-w-[820px]" style={{ tableLayout: 'fixed' }}>
              <thead>
                <tr className="bg-[#4E69D3]">
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[26%]">Patient Name</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[13%]">ID</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[15%]">Service</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[16%]">Date</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[15%]">Time</th>
                  <th className="px-6 py-4 text-left font-bold text-white text-[16px] uppercase tracking-[1px] font-poppins w-[15%]">Actions</th>
                </tr>
              </thead>
              <tbody>
                {futureSamples.slice((upcomingPage - 1) * 10, upcomingPage * 10).map(p => (
                  <tr key={p.ptn + p.iso} className={`border-b last:border-0 transition-colors ${darkMode ? 'odd:bg-[#0f1438] even:bg-white/[0.04] border-white/[0.06] hover:bg-[#3d2768]' : 'odd:bg-white even:bg-[#F6F8FF] border-[#EEF2F7] hover:bg-[#EEF0FB]'}`}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3 overflow-hidden">
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 ${p.relativeOf ? 'bg-[#E8EAF6] text-[#4E69D3]' : darkMode ? 'bg-[#4E69D3]/25 text-blue-200' : 'bg-[#E8EAF6] text-[#4E69D3]'}`}>{p.name.charAt(0)}</div>
                        <div className="min-w-0">
                    ...
                    <td className={`px-6 py-4 text-[16px] font-medium whitespace-nowrap ${darkMode ? 'text-[#F9FAFB]' : 'text-gray-700'}`}>{p.date}</td>
                    <td className={`px-6 py-4 text-[16px] font-medium whitespace-nowrap ${darkMode ? 'text-[#F9FAFB]' : 'text-gray-700'}`}>{p.time}</td>
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        <button onClick={() => openRecord(p.ptn)} className={`bg-transparent border rounded-lg px-3 py-2 text-xs font-semibold cursor-pointer transition-colors whitespace-nowrap ${darkMode ? 'text-[#F9FAFB] border-white/30 hover:bg-white/10' : 'text-[#4E69D3] border-[#4E69D3] hover:bg-[#EEF0FB]'}`}>View Record</button>
                        <button onClick={() => notifyPatient(p.ptn)} className={`border-none rounded-lg px-3 py-2 text-xs font-semibold cursor-pointer transition-colors whitespace-nowrap ${notified.has(p.ptn) ? 'bg-green-600 text-white' : 'bg-[#4E69D3] text-white hover:bg-[#3D56B8]'}`}>{notified.has(p.ptn) ? 'Sent!' : 'Notify'}</button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        {Math.ceil(futureSamples.length / 10) > 1 && (
          <div className="flex justify-end mt-5">
            <Pagination darkMode={darkMode} page={upcomingPage} totalPages={Math.ceil(futureSamples.length / 10)} onPage={setUpcomingPage} />
          </div>
        )}
      </div>
                ...
                <div>
                  ...
                  {searchStatus === 'found' && searchResult && (
                    <div className="mt-4">
                      ...
                      <p className={`mt-3 text-[13px] ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Account found. The patient's details below will be pre-filled &mdash; review and continue.</p>
                    </div>
                  )}
...
function Pagination({ darkMode, page, totalPages, onPage }: { darkMode: boolean; page: number; totalPages: number; onPage: (p: number) => void }) {
  return (
    <div className="flex items-center gap-3">
      <button
        disabled={page <= 1}
