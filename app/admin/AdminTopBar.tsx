...
function NotificationDropdown({ darkMode, onClose }: { darkMode: boolean; onClose: () => void }) {
  const [activeCategory, setActiveCategory] = useState("All");
  const categories = ["All", "Appointment", "Record Management", "Events"];
  const filtered = activeCategory === "All" ? MOCK_NOTIFICATIONS : MOCK_NOTIFICATIONS.filter((n) => n.category === activeCategory);

  return (
    <>
      <div className="fixed inset-0 z-10" onClick={onClose} />
      <div className={`absolute right-0 top-full mt-2 z-20 w-[380px] max-h-[480px] rounded-xl shadow-lg overflow-hidden flex flex-col ${darkMode ? "bg-[#2d1b4e] border border-[rgba(255,255,255,0.10)]" : "bg-white border border-slate-200"}`}>
        <div className={`shrink-0 flex items-center justify-between px-5 py-4 border-b ${darkMode ? "border-[rgba(255,255,255,0.10)]" : "border-slate-200"}`}>
          <h3 className={`font-poppins text-lg font-bold ${darkMode ? "text-[#F9FAFB]" : "text-[#2A2E43]"}`}>Notifications</h3>
          <div className="flex items-center gap-2">
            <button className="text-xs font-semibold text-[#4E69D3] hover:underline">Mark all as read</button>
            <button onClick={onClose} className={darkMode ? "text-gray-300" : "text-slate-400"}>
              <X size={16} />
            </button>
          </div>
        </div>
            ...
            filtered.map((n) => (
              ...
              >
                <div className="mb-1 flex items-center justify-between">
                  <span className="rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white" style={{ background: CATEGORY_COLORS[n.category] }}>
                    {n.category}
                  </span>
                  {n.unread && <span className="h-2 w-2 rounded-full bg-[#4E69D3]" />}
                </div>
                <p className={`text-sm font-bold ${darkMode ? "text-[#F9FAFB]" : "text-[#2A2E43]"}`}>{n.title}</p>
                <p className={`mt-0.5 text-xs leading-relaxed ${darkMode ? "text-gray-300" : "text-gray-500"}`}>{n.description}</p>
          ...
          <>
            ...
            <div
              className={`absolute right-0 top-full mt-2 z-20 w-72 rounded-xl border shadow-lg overflow-hidden ${
                darkMode ? "bg-[#2d1b4e] border-[rgba(255,255,255,0.10)]" : "bg-white border-slate-200"
              }`}
            >
              <div className="flex items-center gap-3 px-4 py-4 border-b border-inherit">
                <div className={`flex h-10 w-10 items-center justify-center rounded-full ${darkMode ? "bg-[#050617] text-gray-300" : "bg-slate-100 text-slate-500"}`}>
                  <UserRound size={20} />
                </div>
                <div className="min-w-0">
                  <div className="text-xs font-bold text-sky-500">{user.id}</div>
                  <div className={`truncate text-sm font-bold ${darkMode ? "text-[#F9FAFB]" : "text-slate-800"}`}>{user.fullName}</div>
                  <div className={`truncate text-xs ${darkMode ? "text-gray-400" : "text-slate-500"}`}>{user.email}</div>
                </div>
              </div>
              ...
              <nav className="py-1">
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-3 text-sm font-semibold ${
                    darkMode ? "text-[#F9FAFB] hover:bg-[#3a2464]" : "text-slate-700 hover:bg-slate-50"
                  }`}
                >
                ...
                <div className="flex items-center justify-between px-4 py-3">
                  <span className={`flex items-center gap-3 text-sm font-semibold ${darkMode ? "text-[#F9FAFB]" : "text-slate-700"}`}>
