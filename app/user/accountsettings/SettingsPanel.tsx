...
export default function SettingsPanel({ user }: { user: SettingsUser }) {
  ...
  const rowClass = `flex items-center justify-between py-3 border-b ${
  ...
  const renderContent = () => {
    switch (active) {
      case 'account':
        return <AccountSection darkMode={darkMode} user={user} />
      case 'notifications':
        return (
          <>
            ...
            ] as [string, string, boolean, () => void][]).map(([label, desc, checked, onChange]) => (
              <div key={label as string} className={rowClass}>
                ...
                <Toggle checked={checked as boolean} onChange={onChange as () => void} />
              </div>
            ))}
          </>
        )
  ...
  const menuPanel = (
    <div className={`w-[260px] flex-shrink-0 ${darkMode ? 'bg-[#3d2266] border-[rgba(255,255,255,0.10)]' : 'bg-white border-[rgba(15,60,95,0.10)]'} border rounded-[18px] py-2 overflow-hidden shadow-[0_4px_6px_-1px_rgba(0,0,0,0.06)] max-[900px]:w-full`}>
      {sections.map((s) => (
        <button
          key={s.id}
          className={`w-full flex items-center gap-3 px-5 py-2.5 border-none bg-transparent text-[15px] font-semibold font-poppins cursor-pointer text-left transition-colors whitespace-nowrap max-[900px]:px-4 max-[900px]:text-[14px] ${
            active === s.id
      ...
      <div className={`h-px mx-5 my-1.5 ${darkMode ? 'bg-[rgba(255,255,255,0.10)]' : 'bg-gray-200'} max-[900px]:mx-4`} />
      <button
  ...
  return (
    <div>
      {isMobile && mobileScreen === 'detail' ? (
        <>
          <button
            className={`flex items-center gap-1 mb-4 px-1 py-2 border-none bg-transparent text-[15px] font-semibold font-poppins cursor-pointer transition-colors ${darkMode ? 'text-[#C4B5FD] hover:text-white' : 'text-[#4E69D3] hover:text-[#2A2E43]'}`}
...
function AccountSection({ darkMode, user }: { darkMode: boolean; user: SettingsUser }) {
  ...
  const reqs = [
    { label: '8+ characters', met: passwordForm.newPass.length >= 8 },
  ...
  useEffect(() => {
    if (state?.success && state.payload) {
      setForm({
        ...
        email: state.payload.email ?? user.email ?? '',
      })
      setEditing(false)
      update({ ...session?.user, ...state.payload })
  ...
  return (
    <>
        ...
        {!editing ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={isPending}
              className="bg-[#4E69D3] text-white px-4 py-2 rounded-md text-sm font-semibold border-none cursor-pointer hover:bg-[#3D56B8] transition-colors disabled:opacity-60"
            >
      ...
      <form id="account-settings-form" action={handleSubmit}>
        <div className="grid grid-cols-2 gap-4 max-[600px]:grid-cols-1">
          <div key="name">
            <p className={`text-[11px] font-bold uppercase tracking-[0.5px] m-0 mb-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Name</p>
            {editing ? (
              <input
                type="text"
                name="name"
              ...
              <p className={`text-[16px] font-semibold m-0 max-[900px]:text-[13px] max-[900px]:truncate ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}>{form.name}</p>
            )}
            {state?.errors?.name && <p className="mt-1 text-[12px] m-0 text-rose-500">{state.errors.name}</p>}
          </div>
          <div key="email">
            <p className={`text-[11px] font-bold uppercase tracking-[0.5px] m-0 mb-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Email</p>
            {editing ? (
              <input
                type="email"
                name="email"
              ...
              <p className={`text-[16px] font-semibold m-0 max-[900px]:text-[13px] max-[900px]:truncate ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}>{form.email}</p>
            )}
            {state?.errors?.email && <p className="mt-1 text-[12px] m-0 text-rose-500">{state.errors.email}</p>}
          </div>
          <div key="role">
            <p className={`text-[11px] font-bold uppercase tracking-[0.5px] m-0 mb-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Role</p>
            <p className={`text-[16px] font-semibold m-0 max-[900px]:text-[13px] max-[900px]:truncate ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}>{user.role ?? 'USER'}</p>
          </div>
          <div key="patientId">
            <p className={`text-[11px] font-bold uppercase tracking-[0.5px] m-0 mb-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Patient ID</p>
            <p className={`text-[16px] font-semibold m-0 max-[900px]:text-[13px] max-[900px]:truncate ${darkMode ? 'text-[#F9FAFB]' : 'text-[#2A2E43]'}`}>{patientId}</p>
          </div>
        </div>
        {state?.message && !state.success && (
          <p className={`text-[14px] font-semibold m-0 mt-3 ${darkMode ? 'text-rose-400' : 'text-rose-600'}`}>{state.message}</p>
        )}
      </form>
            ...
            <div className="grid grid-cols-1 gap-x-4 gap-y-3 lg:grid-cols-3">
              <div>
                ...
                <div className="relative">
                  <input type={showCurrent ? 'text' : 'password'} name="current_password" value={passwordForm.current} onChange={(e) => setPasswordForm((prev) => ({ ...prev, current: e.target.value }))} className={`${fieldClass} pr-10`} placeholder="Enter current password" />
                  <button
                    type="button"
                ...
                {passwordState?.errors?.current_password && (
                  <p className="mt-1 text-[12px] m-0 text-rose-500">{passwordState.errors.current_password}</p>
                )}
              </div>
              <div>
                <p className={`text-[11px] font-bold uppercase tracking-[0.5px] m-0 mb-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>New Password</p>
                <div className="relative">
                  <input type={showNewPass ? 'text' : 'password'} name="new_password" value={passwordForm.newPass} onChange={(e) => setPasswordForm((prev) => ({ ...prev, newPass: e.target.value }))} className={`${fieldClass} pr-10`} placeholder="Enter new password" />
                  <button
                    type="button"
                ...
                {passwordForm.newPass !== '' && (
                  <>
                    ...
                    {passwordForm.newPass === passwordForm.current && (
                ...
                {passwordState?.errors?.new_password && (
                  <p className="mt-1 text-[12px] m-0 text-rose-500">{passwordState.errors.new_password}</p>
                )}
              </div>
              <div>
                ...
                <div className="flex gap-2 items-end">
                  <div className="relative flex-1">
                    <input type={showConfirm ? 'text' : 'password'} name="confirm_password" value={passwordForm.confirm} onChange={(e) => setPasswordForm((prev) => ({ ...prev, confirm: e.target.value }))} className={`${fieldClass} pr-10`} placeholder="Confirm new password" />
                    <button
                      type="button"
                  ...
                  <button
                    ...
                    className="bg-[#4E69D3] text-white px-4 py-2 rounded-md text-sm font-semibold border-none cursor-pointer hover:bg-[#3D56B8] transition-colors whitespace-nowrap disabled:opacity-60"
                  >
                    {passwordPending ? 'Updating…' : 'Update'}
                  </button>
                </div>
                {passwordState?.errors?.confirm_password && (
                  <p className="mt-1 text-[12px] m-0 text-rose-500">{passwordState.errors.confirm_password}</p>
                )}
              </div>
            </div>
          </form>
